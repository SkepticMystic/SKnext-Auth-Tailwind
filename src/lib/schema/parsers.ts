 ...
 */
const request = async <O, D extends z.ZodTypeDef, I>(request: Request, schema: z.ZodSchema<O, D, I>) => {
    const input = await request.json()
    return raw(input, schema)
}
const form = async <O, D extends z.ZodTypeDef, I>(request: Request, schema: z.ZodSchema<O, D, I>) => {
    const form = await request.formData()
    const input = Object.fromEntries(form)
    return raw(input, schema)
}
const params = <O, D extends z.ZodTypeDef, I>(params: URLSearchParams | URL, schema: z.ZodSchema<O, D, I>) => {
